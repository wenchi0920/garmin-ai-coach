...
1. **定位**：判斷該賽事所屬國家的 ISO 3166-1 alpha-3 小寫代碼 (如 twn, jpn, usa)。若目錄不存在，請建立該資料夾並初始化 \`README.md\` (國家索引) 與 \`Honolulu_Marathon.md\` (詳情庫)。
2. **README.md (索引)**：在該國 README.md 表格新增一列。包含：月份、中文名、英文名、特色、賽道認證、詳情連結 (\`Honolulu_Marathon.md#錨點\`)，**賽道認證顯示方式 , 例如 AIMS 認證,WA 白金標, WA 金標,WA 菁英,WA一般, WMM** ，**重要：新增後必須確保表格按月份 (1-12) 重新排序**。
3. **Honolulu_Marathon.md (詳情)**：**重要：請引用當年度最新的資料**在末尾新增四段式結構：### [中文名] ([英文名])。包含「1. 歷史背景」、「2. 賽道技術分析」(含配速策略)、「3. 補給特色」、「4. 教練專業評論」(含 PB 潛力、LSD 定位), 要建立 **[返回國家索引](README.md) | [返回全球總索引](../README.md)**。
4. **專業要求**：內容約 200 字，語氣科學嚴謹，必須使用術語如 PB, BQ, LSD, RPE, 負分割 (Negative Split), 離心收縮。
